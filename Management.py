...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Applications/PsychoPy.app/Contents/Resources/management1-text/Management.py',
...
trial = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('Management.xlsx', selection='1:150'),
...
for thisTrial in trial:
    ...
    loop = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('image/loop.xlsx'),
        seed=None, name='loop')
    ...
    for thisLoop in loop:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        y_feedback.setImage(Questions)
        key_resp_yfeedback.keys = []
        key_resp_yfeedback.rt = []
        _key_resp_yfeedback_allKeys = []
        # keep track of which components have finished
        y_2Components = [y_feedback, key_resp_yfeedback]
        ...
        while continueRoutine:
            ...
            # *y_feedback* updates
            if y_feedback.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                y_feedback.frameNStart = frameN  # exact frame index
                y_feedback.tStart = t  # local t and not account for scr refresh
                y_feedback.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(y_feedback, 'tStartRefresh')  # time at next scr refresh
                y_feedback.setAutoDraw(True)
        ...
        for thisComponent in y_2Components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        loop.addData('y_feedback.started', y_feedback.tStartRefresh)
        loop.addData('y_feedback.stopped', y_feedback.tStopRefresh)
        # check responses
        if key_resp_yfeedback.keys in ['', [], None]:  # No response was made
            key_resp_yfeedback.keys = None
        loop.addData('key_resp_yfeedback.keys',key_resp_yfeedback.keys)
        if key_resp_yfeedback.keys != None:  # we had a response
            loop.addData('key_resp_yfeedback.rt', key_resp_yfeedback.rt)
        loop.addData('key_resp_yfeedback.started', key_resp_yfeedback.tStartRefresh)
        loop.addData('key_resp_yfeedback.stopped', key_resp_yfeedback.tStopRefresh)
